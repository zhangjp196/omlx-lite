...
oMLX Lite is designed for personal use and deployments managed by a trusted administrator.
...
- oMLX Lite does not provide isolation between mutually untrusted tenants.
...
Keep oMLX Lite updated. Configure an API key before exposing the server to a network, including through a reverse proxy or tunnel. Restrict access to intended clients and protect traffic over untrusted networks. Keep cluster communication on a trusted, access-controlled network.
...
Reporters of confirmed vulnerabilities are credited in the relevant fix or advisory unless they prefer to remain anonymous. oMLX Lite does not offer a paid bug bounty.
