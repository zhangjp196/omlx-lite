...
A call's misses are read in parallel with `os.pread` on a shared thread pool;
`ensure()` schedules them before the serial install loop. Slot writes, LRU
updates, and hit/miss counters stay on the calling thread.
